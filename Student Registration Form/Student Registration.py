...
formbg = "#5453a6"
root.geometry("1250x700+10+00")
root.config(bg=formbg)
root.resizable(0,0)

file = pathlib.Path('Student/student_data.xlsx')
if file.exists():
     pass
else:
     file = Workbook()
     ...
     sheet['F1']="GENDER"
     sheet['G1']="Class"
     file.save('Student/student_data.xlsx')
...
def validation():
     ...
     if(nm or ps or cnp or em or mb or gn or cl):
          file = openpyxl.load_workbook('Student/student_data.xlsx')
          sheet = file.active
...
name = StringVar()
name_entry = Entry(obj, textvariable=name, font='arial, 12', width='20' ).place(x=200, y=18)

password = StringVar()
pass_entry = Entry(obj, textvariable=password, font='arial, 12', width='20' ).place(x=200, y=68)

conpass = StringVar()
conpass_entry =  Entry(obj, textvariable=conpass, font='arial, 12', width='20' ).place(x=200, y=118)

email = StringVar()
email_entry =  Entry(obj, textvariable=email, font='arial, 12', width='20' ).place(x=200, y=168)

mobile = StringVar()
mobile_entry = Entry(obj, textvariable=mobile, font='arial, 12', width='20' ).place(x=200, y=218)
